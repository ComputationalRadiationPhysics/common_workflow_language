...
def check_and_clone_repo(picongpu_git_path, picongpu_local_dir, cwl_output_file):
    ...
    if not os.path.isdir(picongpu_local_dir):
        ...
        subprocess.run(['git', 'clone', picongpu_git_path, picongpu_local_dir], stdout=open(cwl_output_file, 'w'), stderr=subprocess.STDOUT)
    else:
...
def main():
    ...
    check_and_clone_repo(picongpu_git_path, picongpu_local_dir, cwl_output_file)
